...
(function () {
  ...
  const els = {
    ...
    // AI Suggestions ("how to improve your site")
    aiSuggestionsLoading: document.getElementById('aiSuggestionsLoading'),
    aiSuggestionsUnavailable: document.getElementById('aiSuggestionsUnavailable'),
    aiSuggestionsUnavailableText: document.getElementById('aiSuggestionsUnavailableText'),
    aiSuggestionsBody: document.getElementById('aiSuggestionsBody'),
    aiSuggestionsSummary: document.getElementById('aiSuggestionsSummary'),
    aiTopPrioritiesList: document.getElementById('aiTopPrioritiesList'),
    aiQuickWinsList: document.getElementById('aiQuickWinsList'),
    aiSuggestionsDisclaimer: document.getElementById('aiSuggestionsDisclaimer'),
  ...
  function renderDashboard(record) {
    ...
    loadAiSuggestions(record);
  }

  // ---- AI Suggestions ("how to improve your site") --------------------------
  // Loads independently of (and after) the main dashboard content above, so a
  // slow or unavailable AI call never blocks the deterministic audit results
  // the user came here for.
  function showAiSuggestionsState(state) {
    if (els.aiSuggestionsLoading) els.aiSuggestionsLoading.hidden = state !== 'loading';
    if (els.aiSuggestionsUnavailable) els.aiSuggestionsUnavailable.hidden = state !== 'unavailable';
    if (els.aiSuggestionsBody) els.aiSuggestionsBody.hidden = state !== 'body';
  }

  function renderAiSuggestionsUnavailable(message) {
    if (els.aiSuggestionsUnavailableText) {
      els.aiSuggestionsUnavailableText.textContent =
        message || 'AI suggestions are unavailable right now. You can still review every finding below.';
    }
    showAiSuggestionsState('unavailable');
  }

  function renderAiSuggestionsBody(analysis) {
    if (els.aiSuggestionsSummary) {
      els.aiSuggestionsSummary.textContent = analysis.executiveSummary || '';
    }

    if (els.aiTopPrioritiesList) {
      const topImprovements = Array.isArray(analysis.topImprovements) ? analysis.topImprovements : [];
      els.aiTopPrioritiesList.innerHTML = topImprovements.length
        ? topImprovements
            .map((rec) => `<li><strong>${escapeHtml(rec.problem)}</strong> — ${escapeHtml(rec.recommendedFix)}</li>`)
            .join('')
        : '<li>No priority issues were found — nice work.</li>';
    }

    if (els.aiQuickWinsList) {
      const quickWins = Array.isArray(analysis.quickWins) ? analysis.quickWins : [];
      els.aiQuickWinsList.innerHTML = quickWins.length
        ? quickWins.map((rec) => `<li>${escapeHtml(rec.recommendedFix)}</li>`).join('')
        : '<li>No fast, low-effort fixes were identified for this audit.</li>';
    }

    if (els.aiSuggestionsDisclaimer) {
      els.aiSuggestionsDisclaimer.textContent =
        analysis.disclaimer ||
        'These suggestions are generated by AI based on the detected data above. They are suggestions, not guarantees of search ranking, traffic, or conversion improvements.';
    }

    showAiSuggestionsState('body');
  }
  ...
  async function loadAiSuggestions(record) {
    showAiSuggestionsState('loading');

    if (record.analysis) {
      renderAiSuggestionsBody(record.analysis);
      return;
    }

    const { analysis, error } = await requestAnalysis(record.scanId);
    // The dashboard may have moved on to a different scan (e.g. the user
    // clicked a different Recent Audit) by the time this resolves.
    if (record.scanId !== currentScanId) return;

    if (analysis) {
      renderAiSuggestionsBody(analysis);
    } else {
      renderAiSuggestionsUnavailable(error);
    }
  }
